...
public class ScheduleIO
    ...
    public static List<Schedule> find(String where, DataConnection dc) {
        return findClass(Schedule.class, where, dc);
    }

    public static List<Schedule> findCourse(String where, DataConnection dc) {
    ...
    static List<Schedule> findClass(Class c, String where, DataConnection dc) {
    ...
    public static List<ScheduleObject> getLoadRS(PreparedStatement ps, DataConnection dc)
            throws SQLException {
    ...
    public static List<ScheduleObject> findObject(String where, DataConnection dc)
            throws SQLException {
        List<ScheduleObject> v = new ArrayList<>();
    ...
    public static void createIndividualFollowUp(int rangeId, FollowUp up, DataConnection dc) throws PlanningException {
        try {
            dc.setAutoCommit(false);
            int noteId = createFollowUp(up, dc);
            List<ScheduleRange> ranges = ScheduleRangeIO.find("pg WHERE pg.id = " + rangeId, dc);
            if (ranges.size() > 0) {
                ScheduleRange r = ranges.get(0);
                r.setNote(noteId);
