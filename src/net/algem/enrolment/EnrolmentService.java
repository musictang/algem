...
public class EnrolmentService
    ...
    /**
     * Gets a list of courses(id, title) scheduled from {@literal startDate} in
     * {@literal estab}. Depending course's code, schedule's type and length may
     ...
     * @throws SQLException
     */
    List<SQLkey> getCoursesFromEstab(int estab, DateFr startDate, CourseModuleInfo courseInfo) throws SQLException {
...
//    int type = CourseCodeType.ATP.getId() == code ? Schedule.WORKSHOP : Schedule.COURSE;
        ...
        List<SQLkey> values = new ArrayList<>();
        try (ResultSet rs = dc.executeQuery(query)) {
            while (rs.next()) {
                values.add(new SQLkey(rs.getInt(1), rs.getString(2)));
            }
        }
     ...
     */
    List<Schedule> getCourseWeek(Course c, DateFr start, int idEstab) {
...
//    int type = c.isATP() ? Schedule.WORKSHOP : Schedule.COURSE;
    ...
    List<Schedule> getCourseWeek2(Course c, DateFr start, int idEstab, int day, int teacher) {
...
//    int type = c.isATP() ? Schedule.WORKSHOP : Schedule.COURSE;
     ...
     * @see net.algem.enrolment.CourseEnrolmentDlg
     */
    List<Schedule> getCourseDay(int course, int day, int code, DateFr start, int estab) {
    ...
    List<Schedule> getCourseDay2(int course, int day, int code, DateFr start, int estab, int teacher) {
    ...
    /**
     * Gets the date when the member {@literal member} is scheduled for the
    ...
    /**
     * Gets the first course {@literal courseId} scheduled from the
     * {@literal date} selected.
     *
    ...
    /**
     * Used in {@link net.algem.enrolment.CourseEnrolmentDlg} for time slot
     * verification.
     *
     * @param p schedule
    ...
    List<ScheduleRange> getBusyTimeSlot(int idAction, int idCourse, String start, String end) {
     ...
     * @throws java.sql.SQLException
     */
    public List<CourseOrder> getCourseOrder(int i, int m) throws SQLException {
     ...
     */
    public static List<PersonFile> findMembersByPlanning(Schedule plan) {
...
//    return ((PersonFileIO) DataCache.getDao(Model.PersonFile)).findByIdOrder(query, false);
    ...
    /**
     * Gets the first scheduled course {@literal c} from {@literal start} in the
     * establishment {@literal estab}.
     *
     * @param idCours
     * @param start
     * @param estab
...
//    int type = c.isATP() ? Schedule.WORKSHOP : Schedule.COURSE;
        ...
        List<Schedule> v = ScheduleIO.find(query, dc);
        if (v.size() > 0) {
            return v.get(0); // retourne un seul planning
        } else {
            return null;
        }
    }
    ...
    /**
     * Checks if member width the id {@code m} is already scheduled between
     * start of schedule {@code p} and end of enrolment.
     *
     * @param m member id
     * @param p schedule
     * @param courseOrder
    ...
    boolean isOnRange(int m, Schedule p, CourseOrder courseOrder) throws EnrolmentException {
        ...
        try {
            String where = "pg WHERE pg.adherent = " + m + " AND pg.idplanning IN("
                    ...
                    + "')";
            List<ScheduleRange> vp = ScheduleRangeIO.find(where, dc);
            if (vp.size() > 0) {
                return true;
            }
    ...
    private void insertRange(CourseOrder co, Course c, List<Schedule> v, ScheduleRange h)
    ...
    int updateRange(ModuleOrder module, CourseOrder co, int memberId) throws SQLException {
        ...
        List<Schedule> v = ScheduleIO.findCourse(query, dc);
        if (v.size() < 1) {
            return 0;
        }
    ...
    void delete(ModuleOrder mo, int member) throws EnrolmentException {
        try {
            ...
            List<CourseOrder> courseOrders = CourseOrderIO.find("WHERE cc.module = " + mo.getId(), dc);
            for (int i = 0; i < courseOrders.size(); i++) {
                CourseOrder cc = courseOrders.get(i);
     ...
     * @param redefine If true, a new course order to define is created. If
     * false, course order is only updated.
     * @throws EnrolmentException
     */
    ...
    /**
     * Stops a module and delete (if exist) the schedule ranges after the date
     * of {@literal stop}.
     *
    ...
    /**
     * Gets the time length of the sessions already performed by the member
     * {@literal idper} between {@code start} and {@code end}, corresponding to
     * the module order {@literal mOrderId}. A session is seen as completed if
     * it was scheduled, even though it has not actually occurred.
     *
     * @param idper member's id
     * @param mOrderId id of the module order corresponding to the training
     * performed
     * @param start start date
     * @param end end date
    ...
    /**
     * Gets the time length of the sessions already performed by the member
     * {@literal idper} from {@code start}, corresponding to the module order
     * {@literal mOrderId}. A session is seen as completed if it was scheduled,
