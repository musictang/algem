        ...
        implements ChangeListener, ItemListener, PersonFileListener //GemEventListener
{
    ...
    private MemberEditor memberEditor; //CACHE
    ...
    public void init() {
        ...
        if (dossier.getContact() != null) {
            ...
            if (dossier.getPhoto() == null) {
                if (Person.PERSON == dossier.getContact().getType() || Person.ROOM == dossier.getContact().getType()) {
                    BufferedImage img = ImageUtil.getPhoto(dossier.getContact().getId());
                    ImageIcon icon = (img == null ? null : new ImageIcon(img));
                    dossier.setPhoto(icon);
                }
            }
            // Rest on last subscription
            if (dossier.getSubscriptionCard() == null) {
                PersonSubscriptionCard lastCard = memberService.getLastSubscription(dossier.getId(), true);
                dossier.setSubscriptionCard(lastCard);
            }
            contactFileEditor.setSubscriptionRest(dossier.getSubscriptionCard());
        }
        ...
        if (dossier.getMember() != null) {
            ...
            if (dossier.getContact().getAddress() == null && dossier.getContact().getTele() == null) {
                ...
                if (Algem.isFeatureEnabled("cc-mdl")) { //ERIC 2.17
                    p = DataCache.getPersonFile(dossier.getMember().getFamily()); // ERIC 3 22/10/2021
                    if (p == null) {
                        MessagePopup.warning(this, MessageUtil.getMessage("not.existing.family.link"));
                        GemLogger.log("Error PersonFileTabView.init family ID NOT FOUND:" + dossier.getMember().getFamily());
                    }
                } else {
                    p = DataCache.getPersonFile(dossier.getMember().getPayer());
                    if (p == null) {
                        MessagePopup.warning(this, MessageUtil.getMessage("not.existing.payer.link"));
                        GemLogger.log("Error PersonFileTabView.init payer ID NOT FOUND:" + dossier.getMember().getPayer());
                    }
                }
                if (p != null) {
                    if (p.getContact().getAddressAll().size() > 0 || p.getContact().getTele().size() > 0) {
                        ...
                        // adherent lié info
                        contactFileEditor.setLinkTelAddress(p.getContact().getAddressAll(), p.getContact().getTele(), cbTelAdresse);
                    }
                }
            }
...
//      teacherEditor.load();
    ...
    /**
     * @param evt event
     * @see net.algem.contact.PersonFileListener (Used only for subscription
     * card).
     */
    @Override
...
//      ribView.setRib(b);
    ...
    /**
     * Adds a member tab. If the member is a student, follow up and enrolment
     * tabs are also added.
     *
     * @see #completeMember()
     */
...
//    wTab.setSelectedIndex(0);
    ...
    /**
     * Adds a tab for groups the member belongs to. Only contacts of type
