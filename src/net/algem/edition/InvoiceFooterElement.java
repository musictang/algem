/*
 * @(#)InvoiceFooterElement.java 2.6.a 17/09/12
 *
        ...
        extends InvoiceItemElement
{
  ...
  public InvoiceFooterElement(int x, int y, Quote f) {
    super(x, y);
    this.f = f;
  }
  ...
  public void draw(Graphics g) {
    ...
    int sep2 = sep1 + 100;
    int acompteOffset = f.hasDownPayment() ? 40 : 0;
    ...
    for (Map.Entry<String, Double> entry : f.getTotalVAT().entrySet()) {
    ...
    rightAlign(g, f.getTotalET(), sep2, y + 72);
    rightAlign(g, (f.getTotalATI()), end, y + 72);

    if (f.hasDownPayment()) {
      g.drawString(BundleUtil.getLabel("Invoice.down.payment.label"), sep1 + 5, y + 92);
      rightAlign(g, -(f.getDownPayment()), end, y + 92);
      g.drawString(BundleUtil.getLabel("Invoice.net.label"), sep1 + 5, y + 112);
      rightAlign(g, (f.netToPay()), end, y + 112);
    }
  }
}
