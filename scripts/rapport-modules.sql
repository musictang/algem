...
-- GOOD échéances encaissées par numéro de commande
...
-- echéances par numéro de commande
SELECT m.titre, (sum(montant) / 100.00)::numeric(10,2) from echeancier2 e join commande c on e.commande = c.id join commande_module cm on c.id = cm.idcmd  join module m on cm.module = m.id where e.echeance >= '18-04-2017' and e.montant > 0 and e.reglement <> 'FAC' and e.compte not in (19,20) group by m.titre order by m.titre;
