/*
 * @(#)TestMemberCard.java 2.6.a 08/10/12
 * 
 * Copyright (c) 1999-2011 Musiques Tangentes. All Rights Reserved.
 *
 * This file is part of Algem.
 * Algem is free software: you can redistribute it and/or modify it
 * under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Algem is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with Algem. If not, see <http://www.gnu.org/licenses/>.
 * 
 */

package net.algem.edition;

import junit.framework.TestCase;
import net.algem.util.ImageUtil;


/**
 *
 * @author <a href="mailto:jmg@musiques-tangentes.asso.fr">Jean-Marc Gobat</a>
 * @version 2.6.a
 */
public class TestMemberCard extends TestCase {
    
    public TestMemberCard(String testName) {
        super(testName);
    }

    @Override
    protected void setUp() throws Exception {
        super.setUp();
    }

    @Override
    protected void tearDown() throws Exception {
        super.tearDown();
    }

    public void testToMM() {
      int n = 72; //1 inch
      double expected = 25.4;
      double r = ImageUtil.toMM(n);
      assertTrue("expected = "+expected+"; current = "+r,r == expected);
    }

    public void testToInch() {
      double n = 25.4d;
      double expected = 1;
      double r = ImageUtil.toInch(n);
      assertTrue("expected = "+expected+"; current = "+r,r == expected);
    }

    public void testToPoints() {
      double n = 25.4;
      int expected = 72;
      double r = ImageUtil.mmToPoints(n);
      assertTrue("expected = "+expected+"; current = "+r,r == expected);
    }



}
